...
/***************************************************************************//**
 * @class Test_SHLPerceptron
 * @brief This class is the Unit Test for SHLPerceptron Class
 * @ingroup Test_NeuralNetPackage
 ******************************************************************************/
class Test_SHLPerceptron : public UnitTestPackage {

public:

    /*************************************************************************//**
     * Constructs a Test_SHLPerceptron object
     ****************************************************************************/
    // Default constructor
    Test_SHLPerceptron() {
        UnitTestPackage();
        ...
        weights[1] = -0.45;
        weights[2] = 0.3;
        _nrn = new SHLPerceptron(3, weights, true);
    ...
    void Run_Tests(void) {
        ...
        Generate_TestLog("SHLPerceptron");
    }

    // Tests
    void test_ActvF(void) {
        //Tests the ActvF calculation of the SHLPerceptron class
        AssertAlmostEqual(_nrn->ActvF(0.5), 1.0, 1.0e-8, "Error in ActvF");
        AssertAlmostEqual(_nrn->ActvF(0.0), 0.0, 1.0e-8, "Error in ActvF");
        AssertAlmostEqual(_nrn->ActvF(-0.5), -1.0, 1.0e-8, "Error in ActvF");
    }

    void test_dActvF_du(void) {
        //Tests the dActvF_du calculation of the SHLPerceptron class
        double eps_ = 1.e-6;
        double dActvF_du_;
    ...
    void test_Neural_Clone(void) {
        ...
        AssertAlmostEqual(percnrn->ActvF(0.0), 0.0, 1.0e-8, "Error in ActvF");
        AssertAlmostEqual(percnrn->ActvF(-0.5), -1.0, 1.0e-8, "Error in ActvF");
    }
